...
public class DeletionSwipeHelper extends ItemTouchHelper.SimpleCallback
{
    ...
    public DeletionSwipeHelper(int dragDirs, int swipeDirs, Context context, OnSwipeListener listener)
    {
        ...
        backgroundColor = ContextCompat.getColor(context, R.color.sweaper_del);
        deleteColor = ContextCompat.getColor(context, R.color.sweaper_del);
        iconColorFilter = deleteColor;
     iconPadding = res.getDimensionPixelSize(R.dimen.del_padding);
    }
    ...
    public void onChildDraw(Canvas canvas, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive)
    {
        ...
        deleteIcon = ContextCompat.getDrawable(context, R.drawable.disposal);
        ...
        if (deleteIcon != null)
        {
            float cx = right - 10 - deleteIcon.getIntrinsicWidth() / 2f;
            ...
            deleteIcon.setColorFilter(new PorterDuffColorFilter(ContextCompat.getColor(context, R.color.et_hint), PorterDuff.Mode.SRC_IN));
            if (circleRadius > 0f)
            {
